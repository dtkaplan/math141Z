...
#'
#' @importFrom ggformula gf_line
#' @importFrom rlang eval_tidy quos
#' @importFrom ggformula %>%
...
#' @param npts number of points at which to evaluate the function.
...
#'   gf_dhistogram(density ~ age, data = HELPrct, binwidth = 3, alpha = 0.6) %>%
...
plot_f1 <- function(object = NULL, formula, ..., inherit = FALSE, npts = 101) {
  ...
  args_for_gf <- c(list(object, formula, data = Pts),
                   ...
                   inherit = inherit)
  do.call(ggformula::gf_line, args_for_gf)
